...
- If you work with a cloned copy (remotely) you will need to install [Git Credential Manager](http://microsoft.github.io/Git-Credential-Manager-for-Windows/Docs/CredentialManager.html). The three commands you might need if your work with a remote copy is `git add` `git commit` and `git push`. Note that at this point, your changes will only be saved into your fork.
